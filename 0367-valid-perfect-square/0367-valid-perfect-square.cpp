...
public:
    bool isPerfectSquare(int num) {
        ...
        if (num <= 1) return true; // Handle the corner case where num is 0 or 1, which are perfect squares.

        while (s <= e) {
            int64_t mid = s + (e - s) / 2;  // Calculate the middle number within the range. Use int64_t to avoid overflow.
